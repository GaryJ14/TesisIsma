...
const TblContenidoCliente = ({ onSelectSong }) => {
  ...
  const toggleFavoritoHandler = async (contenidoId) => {
    try {
      ...
      if (response.status === 'agregado') {
        Swal.fire('Agregado!', 'Contenido agregado a favoritos.', 'success');
        setFavoritoIds((prev) => [...prev, contenidoId]);
      } else if (response.status === 'eliminado') {
        Swal.fire('Eliminado!', 'Contenido eliminado de favoritos.', 'success');
        setFavoritoIds((prev) => prev.filter(id => id !== contenidoId));
      } else {
        Swal.fire('Error', 'Problema al modificar favorito.', 'error');
      }
    } catch (error) {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const contenidosResponse = await axios.get('http://localhost:8000/api/contenidos/mis-contenidos/', {
  ...
  if (loading) return <p style={{ color: 'white' }}>Cargando tus contenidos...</p>;
  if (error) return <p style={{ color: 'red' }}>Error: {error}</p>;
  if (contenidos.length === 0) return <p style={{ color: 'white' }}>No has subido ningún contenido aún.</p>;
    ...
    <div style={{ paddingBottom: '100px', color: 'white' }}>
      ...
      {contenidos.map((contenido, index) => (
        <div
          ...
          onClick={() => onSelectSong && onSelectSong(contenido)}
        >
...
export default TblContenidoCliente;
